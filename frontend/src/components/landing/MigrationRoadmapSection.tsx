...
export const MigrationRoadmapSection: React.FC = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="min-h-[100svh] w-full flex flex-col justify-center py-8 lg:py-12 pt-20 px-4 sm:px-6 bg-[#F5F5F7] overflow-hidden"
    >
      <div className="max-w-[1140px] w-full mx-auto my-auto">
        ...
        <div
          className="bg-white rounded-2xl sm:rounded-3xl border border-[#E5E5EA] shadow-apple-md p-4 sm:p-6 transition-all duration-700 delay-100 ease-out"
          style={{
            opacity: prefersReduced || inView ? 1 : 0,
        ...
        >
            ...
            {waves.map((w, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-[150px] p-3 rounded-2xl text-left transition-all duration-200 border ${
                    isSelected
          ...
          {/* Panoramic Wave Detail Panel */}
          <div
            key={currentWave.id}
            className="bg-[#F5F5F7] rounded-2xl p-5 sm:p-6 border border-[#E5E5EA] transition-all duration-300 ease-out"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3.5 border-b border-[#E5E5EA]">
              <div>
                <span className="text-xs font-mono font-bold text-[#007AFF] uppercase">
                  {currentWave.label} Execution Blueprint
                </span>
            ...
            <div className="py-3.5 space-y-3">
              ...
              <div className="pt-1 space-y-2">
                <span className="text-xs font-bold text-[#1D1D1F] uppercase tracking-wider block font-geist-mono">
                  Prescribed Actions:
                </span>
                {currentWave.keyActions.map((action) => (
                  <div key={action} className="flex items-center gap-2.5 text-xs text-[#1D1D1F]">
                    <div className="w-4.5 h-4.5 rounded-full bg-[#E8F9ED] flex items-center justify-center shrink-0">
