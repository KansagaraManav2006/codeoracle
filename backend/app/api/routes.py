...
def _download_name(value: str) -> str:
    return "".join(character if character.isalnum() or character in "-_" else "-" for character in value).strip("-") or "project"
...
def download_project_analysis(project_id: str, db: Session = Depends(get_db)) -> PlainTextResponse:
    """Downloads a concise, portable Markdown codebase explanation."""
    project = db.query(Project).filter(Project.id == project_id).first()
    if not project:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found.")
...
def download_project_dependency_graph(project_id: str, db: Session = Depends(get_db)) -> PlainTextResponse:
    """Downloads the internal module graph as portable Mermaid Markdown."""
    project = db.query(Project).filter(Project.id == project_id).first()
    if not project:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found.")
    record = db.query(ProjectAnalysisRecord).filter(ProjectAnalysisRecord.project_id == project_id).first()
    ...
    lines.extend(["```", "", f"Connections: {graph.summary.total_edges} | Dependency loops: {graph.summary.cycle_count}", ""])
    return PlainTextResponse(
        "\n".join(lines),
        media_type="text/markdown; charset=utf-8",
...
def load_demo_benchmark(
    ...
    """Ingests a bundled trusted benchmark project with trusted execution flags."""
